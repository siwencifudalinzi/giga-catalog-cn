...
本地采集器优先处理最新影片和近期发生变化的链接；历史回填可使用 1–8 个隔离的持久 Chrome profile 并发、逐条 checkpoint。截至 2026-08-24，3122 个 slot 全部获得终态：3030 个已验证公开落地页，83 个持续转向已停服/未支持目标，9 个被 Cloudflare 人机验证关闭。
...
注意：slot 后缀表示表格中原始列，`entry.provider` 表示实际验证后的落地域。历史表格存在“Streamtape 列实际转到 Player4me”的旧标签；缓存仍严格绑定 `code + slot + sourceUrlHash`，但按实际终点域生成 provider 和按钮文案。
...
记录 `blocked-human`、`retryable`、`unsupported`、`dead`、重试次数、最终域名和错误分类，不进入 `public/`。未知终点连续 4 次不能通过 allowlist 时进入 `unsupported`，防止每日无限重试已停服域名。
...
默认每日浏览器采集最多 100 条，4 个隔离的持久 profile 并发，重叠任务按 `IgnoreNew` 丢弃。成功结果每条立即 checkpoint；真人验证记为 `blocked-human`，未知终点第 4 次仍失败时记为 `unsupported`。公开 manifest 只由完整校验通过的结果原子生成。
...
已从最新样本小流量验证扩大到全部 3122 个 slot。回滚只需回退包含 `resolved-links.json` 的数据提交；权威 `catalog.json` 和原短链从未被改写。

发布开关位于前端配置：关闭时完全忽略 `resolved-links.json`，恢复原始短链行为。回滚只需关闭开关或回退包含缓存文件的提交，不修改权威 `catalog.json`，因此不会破坏每日目录和链接同步。

## 12. 完成标准

- 3122 个 slot 全部有终态，所有能通过 allowlist 验证的公开落地页均进入 manifest。
- 已停服、未支持或人机验证条目继续使用 catalog 中的原短链回退，不伪造直达地址。
